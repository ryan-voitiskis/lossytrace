#!/usr/bin/env python3
"""Validate the bounded sparse non-tonal metadata-search checkpoint."""
...
def validate_plan(plan: dict[str, Any]) -> list[str]:
    ...
    if plan.get("state") != "bounded_metadata_only_sparse_non_tonal_successor_search_frozen_before_query_execution":
    ...
    if Decimal(str(eligibility.get("minimum_duration_seconds"))) != Decimal("2.000"):
        errors.append("minimum duration differs")
    if set(eligibility.get("excluded_exact_member_ids", [])) != EXPECTED_EXCLUDED_IDS:
    ...
    if protocol.get("primary_record_hosts") != ["freesound.org", "zenodo.org"]:
