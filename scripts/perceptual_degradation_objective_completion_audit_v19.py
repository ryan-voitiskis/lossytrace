#!/usr/bin/env python3
"""Refresh the objective audit after sparse non-tonal confirmation v3."""
...
PLAN_PATH = ROOT / "benchmarks/perceptual-degradation-v1/objective-completion-audit-plan-20260819-019.json"
REPORT_PATH = ROOT / "research/toolchains/evidence/perceptual-degradation-objective-completion-audit-20260819-019.json"
PLAN_ID = "perceptual-degradation-objective-completion-audit-20260819-019"
REPORT_ID = PLAN_ID
EXPECTED_BINDINGS = {
    "confirmation_audit",
    "confirmation_plan",
...
NEXT_GATE = (
    "Freeze a separately bounded metadata-only sparse non-tonal v4 search before any further audio access. Preserve all three failed "
    "exact candidates and the unchanged descriptor, exclude every consumed exact member, and target one provider-original lossless "
    "broadband one-shot whose primary record explicitly documents several seconds of quiet before and after the event, the exact "
    "capture chain, permissive rights and no normalization, fade or other processing."
)
...
def validate_plan(plan: dict[str, Any]) -> list[str]:
    ...
    if plan.get("state") != "score_blind_completion_audit_refresh_after_sparse_non_tonal_exact_member_v3_abstention":
...
def _verify_confirmation(plan: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    runner = _load_module(_bound(plan, "confirmation_runner"), "sparse_non_tonal_confirmation_v3")
    runner_errors = runner.validate_plan(confirmation_plan)
    if runner_errors:
        raise ValueError("confirmation plan invalid: " + "; ".join(runner_errors))
    validator = _load_module(_bound(plan, "confirmation_validator"), "sparse_non_tonal_confirmation_v3_validator")
    ...
    if confirmation_audit["decision"] != {
        "candidate_replacement_performed": False,
        "descriptor_confirmation_passed": False,
...
def build_report(plan: dict[str, Any], plan_path: Path = PLAN_PATH) -> dict[str, Any]:
    ...
    predecessor = _load_module(_bound(plan, "predecessor_audit_implementation"), "objective_v18")
    ...
    requirements["truth_bearing_source_manifest_feasible_and_frozen"].update({
        "state": "tonal_non_sparse_exact_contrast_confirmed_three_sparse_non_tonal_candidates_failed_successor_and_manifest_unfrozen",
        "satisfied": False,
        "evidence": [
            "predecessor_audit.requirements.truth_bearing_source_manifest",
            "sparse_non_tonal_exact_member_confirmation_v3.decision",
            "sparse_non_tonal_exact_member_confirmation_v3.execution",
            "sparse_non_tonal_exact_member_confirmation_v3_audit.gates",
            "sparse_non_tonal_exact_member_confirmation_v3_audit.decision",
        ],
        "reason": "Freesound sound 476736 matched the precommitted provider-original container and completed two byte-identical private replays, but it abstained under the unchanged sparse non-tonal descriptor. The independent audit verified the private predicates and exact redacted public terminal result. The candidate was not replaced, no threshold changed, and assignment, relationships and manifest allocation remain unfrozen.",
        "next_gate": NEXT_GATE,
    })
    requirements["difficult_natural_and_production_negatives_preserved_in_evaluation"].update({
        "state": "one_exact_tonal_contrast_confirmed_three_sparse_candidates_failed_successor_and_evaluation_allocation_missing",
        "satisfied": False,
        "evidence": [
        ...
        "reason": "The exact tonal non-sparse contrast remains technically confirmed, while the first and third sparse candidates abstained and the second produced a class mismatch. Neither contrast is assigned or allocated to scientific evaluation, and all three negative candidates remain preserved without post-observation reclassification.",
        "next_gate": NEXT_GATE,
    })
    ...
    if satisfied != 4 or all(row["satisfied"] for row in report["requirements"]):
        raise ValueError("exact-member abstention unexpectedly promoted completion")
    ...
    report.update({
        ...
        "report_id": REPORT_ID,
        "state": "objective_incomplete_third_sparse_candidate_abstention_reconciled_without_trait_or_scientific_promotion",
    })
    report["summary"].update({
        ...
        "sparse_non_tonal_exact_member_confirmation_v3_audio_accessed": True,
        "sparse_non_tonal_exact_member_confirmation_v3_complete": True,
        "sparse_non_tonal_exact_member_confirmation_v3_passed": False,
        "sparse_non_tonal_exact_member_confirmation_v3_terminal_outcome": "abstained",
        "sparse_non_tonal_failed_exact_candidate_count": 3,
    ...
    report["evidence_checks"].update({
        ...
        "sparse_non_tonal_exact_member_confirmation_v3_audio_accessed": True,
        "sparse_non_tonal_exact_member_confirmation_v3_complete": True,
        "sparse_non_tonal_exact_member_confirmation_v3_pass_count": 0,
        "sparse_non_tonal_exact_member_confirmation_v3_plan_frozen": True,
        "sparse_non_tonal_exact_member_confirmation_v3_terminal_abstained": True,
        "sparse_non_tonal_exact_member_confirmation_v3_private_replays_byte_identical": confirmation["execution"]["private_reports_byte_identical"],
        "sparse_non_tonal_exact_member_confirmation_v3_public_projection_independently_audited": (
