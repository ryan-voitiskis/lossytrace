#!/usr/bin/env python3
"""Run a source-paired CRNN lossy-history probe with domain holdouts."""
...
LSTM_LAYERS = 2
PAIRWISE_MARGIN = 1.0
PAIRWISE_LOSS_WEIGHT = 2.0
CLASS_TO_LABEL = {"negative": 0, "controlled_positive": 1}
MODEL_DEFINITION = {
    "schema_version": 1,
    "paper": "Koops, Micchi, and Quinton, ISMIR 2024",
    "reproduction_boundary": "paper_architecture_with_predeclared_source_paired_loss",
    ...
    "mask": {
        ...
        "pairing": "same cutoff for paired PCM and lossy examples",
    ...
    "classification": {
        "outputs": 2,
        "training_loss": {
            "case_loss": "mean cross_entropy over paired PCM and lossy examples",
            "pairwise_loss": "softplus(margin - (lossy_log_odds - pcm_log_odds))",
    ...
    "pairing": {
        "unit": "same source_group",
        "negative_eligibility": "generated PCM controls only",
        "positive_eligibility": "all controlled lossy-to-lossless variants",
        "crop": "same deterministic relative crop in both variants",
        "epoch_plan": "all eligible negative-positive pairs once per epoch",
    },
    "track_inference": {
        "window_seconds": 2,
...
def source_paired_epoch_plan(
    cases: list[dict[str, Any]],
    seed: int,
    epoch: int,
) -> list[tuple[dict[str, Any], dict[str, Any], int]]:
    by_group: dict[str, dict[int, list[dict[str, Any]]]] = defaultdict(
        lambda: defaultdict(list)
    )
    for case in cases:
        if case["_label"] == 0 and not case["relative_path"].startswith("generated/"):
            continue
        by_group[case["partition_group"]][case["_label"]].append(case)
    incomplete = [
        group for group, by_label in by_group.items() if set(by_label) != {0, 1}
    ]
    if incomplete:
        raise ValueError(
            f"source groups lack pairable PCM or positive cases: {incomplete[:3]}"
        )
    rng = random.Random(stable_hash_int(seed, "epoch-plan", epoch))
    result: list[tuple[dict[str, Any], dict[str, Any], int]] = []
    for _, by_label in sorted(by_group.items()):
        negatives = sorted(by_label[0], key=lambda case: case["case_id"])
        positives = sorted(by_label[1], key=lambda case: case["case_id"])
        for occurrence, (negative, positive) in enumerate(
            (negative, positive)
            for negative in negatives
            for positive in positives
        ):
            result.append((negative, positive, occurrence))
    rng.shuffle(result)
    return result
    ...
    class LossyHistoryCrnn(nn.Module):
        def __init__(self) -> None:
            ...
            with torch.no_grad():
                ...
                features = self._cnn_features(shape_probe)
        ...
        def _cnn_features(self, waveforms, *, mask_cutoff_hz=None):
            ...
            if mask_cutoff_hz is not None:
                cutoff = mask_cutoff_hz.to(
                    device=features.device,
                    dtype=features.dtype,
                ).view(-1, 1, 1)
                if cutoff.shape[0] != features.shape[0]:
                    raise ValueError("mask cutoff count differs from batch size")
        ...
        def forward(self, waveforms, *, mask_cutoff_hz=None):
            features = self._cnn_features(
                waveforms,
                mask_cutoff_hz=mask_cutoff_hz,
            )
...
def source_paired_objective(logits, pair_count: int, criterion, torch):
    if pair_count < 1 or logits.shape != (2 * pair_count, 2):
        raise ValueError("paired logits must have shape (2 * pair_count, 2)")
    labels = torch.cat(
        (
            torch.zeros(pair_count, dtype=torch.long, device=logits.device),
            torch.ones(pair_count, dtype=torch.long, device=logits.device),
        )
    )
    case_loss = criterion(logits, labels)
    log_odds = logits[:, 1] - logits[:, 0]
    negative_log_odds = log_odds[:pair_count]
    positive_log_odds = log_odds[pair_count:]
    pairwise_loss = torch.nn.functional.softplus(
        PAIRWISE_MARGIN - (positive_log_odds - negative_log_odds)
    ).mean()
    combined = case_loss + PAIRWISE_LOSS_WEIGHT * pairwise_loss
    return combined, case_loss, pairwise_loss
...
def make_training_dataset_class(Dataset):
    class TrainingDataset(Dataset):
        ...
        def __getitem__(self, index):
            negative, positive, occurrence = self.plan[index]
            fraction = crop_start_fraction(
                self.seed,
                self.epoch,
                negative["partition_group"],
                occurrence,
            )
            negative_waveform = read_crop(
                negative,
                fraction,
                np=self.np,
                sf=self.sf,
            ...
            return negative_waveform, positive_waveform
...
def run_fold(
    ...
    *,
    ...
    for epoch in range(1, args.max_epochs + 1):
        plan = source_paired_epoch_plan(training, fold_seed, epoch)
        ...
        for negative_waveforms, positive_waveforms in loader:
            ...
            waveforms = torch.cat((negative_waveforms, positive_waveforms), dim=0)
            ...
            loss, case_loss, pairwise_loss = source_paired_objective(
                logits,
                pair_count,
                criterion,
                torch,
            ...
            total_case_loss += float(case_loss.detach().cpu()) * pair_count
            total_pairwise_loss += (
        ...
        epochs.append(
            {
                ...
                "training_case_loss": training_case_loss,
                "training_pairwise_loss": training_pairwise_loss,
        ...
        print(
            ...
            f"case_loss={training_case_loss:.6f} "
            f"pair_loss={training_pairwise_loss:.6f} "
...
def validate_only(args) -> dict[str, Any]:
    ...
    source_pairs = source_paired_epoch_plan(cases, args.seed, 1)
    return {
        "case_count": len(cases),
        "source_group_count": len({case["source_group"] for case in cases}),
        "source_pair_count_per_complete_epoch": len(source_pairs),
        "source_pair_group_count": len(
            {negative["source_group"] for negative, _, _ in source_pairs}
        ),
...
def run(args) -> None:
    ...
    report = {
        ...
        "method": "source_paired_crnn_random_mask",
        "reproduction_boundary": (
            "paper_architecture_with_predeclared_source_paired_loss"
        ),
        ...
        "training": {
            ...
            "examples_per_full_batch": 2 * args.batch_size,
            "case_loss": "cross_entropy",
            "pairwise_loss": "softplus_margin_log_odds",
            "pairwise_margin": PAIRWISE_MARGIN,
            "pairwise_loss_weight": PAIRWISE_LOSS_WEIGHT,
            "shared_pair_crop": True,
            "shared_pair_frequency_mask": True,
