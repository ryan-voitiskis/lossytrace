#!/usr/bin/env python3
"""Score-free v5: exact opt-in global and local native correlations.

Frozen predecessors and their evidence are unchanged. No automatic build,
real-audio execution, normalization or perceptual scoring is enabled here.
"""
...
RECORD_KIND = "perceptual_degradation_alignment_v5"
# Explicitly installed by the synthetic qualification runner only; no auto-build.
NATIVE_BACKEND = None
...
def local_lag(
    ...
    if NATIVE_BACKEND is not None:
        results = NATIVE_BACKEND.results(reference, test, global_lag, center, half_window, radius)
    else:
        results = {}
...
def _channel(reference: Sequence[float], test: Sequence[float], rate: int, minimum_active: float, maximum_delay: float) -> dict[str, Any]:
    ...
    sampled = (NATIVE_BACKEND.global_results(reference, test, lags) if NATIVE_BACKEND is not None
               else {lag: correlate(reference, test, lag) for lag in lags})
