...
def derive(module, replays, selected):
    ...
    for replay in replays:
        ...
        for case in replay["cases"]:
            ...
            global_record = case["native_global_audit"]
            if case["state"] == "observed" and global_record["fallback_candidates"] == 0:
                assert global_record["native_candidates"] + global_record["short_overlap_candidates"] == sum(
                    row["candidate_count"] * row["search_count"] for row in global_record["searches"])
        ...
        summaries.append({
            ...
            "global_native_candidates": sum(c["native_global_audit"]["native_candidates"] for c in replay["cases"] if c["native_global_audit"]),
            "global_fallback_candidates": sum(c["native_global_audit"]["fallback_candidates"] for c in replay["cases"] if c["native_global_audit"]),
            "global_short_overlap_candidates": sum(c["native_global_audit"]["short_overlap_candidates"] for c in replay["cases"] if c["native_global_audit"]),
    ...
    result = {
        ...
        "no_native_fallback_all_cases": observed and all(c[name]["fallback_candidates"] == 0 for c in cases for name in ("native_audit", "native_global_audit")),
    ...
    result["primary_technical_gate_passed"] = (observed and repeated and edges
        and result["both_controls_technically_supported_both_rounds"] and result["no_native_fallback_all_cases"])
...
def audit(module, run, source_root, ci, head):
    ...
    marker = json.loads(regular(run.parent / (module.PROPOSAL_ID + ".launch.json")))
    assert marker == {"proposal_id": module.PROPOSAL_ID, "head": head, "ci_run_id": str(ci),
                      "proposal_sha256": sha(module.PLAN.read_bytes()), "automatic_retry_authorized": False}
    ...
    return {"schema_version": 1, "independent_artifact_audit_passed": True,
            ...
            "exclusive_launch_marker_verified": True,
...
def self_test(module):
    ...
    from test_perceptual_degradation_digital_canary_v5 import CanaryTest
    test = CanaryTest()
    test.setUp()
    try:
...
if __name__ == "__main__":
    ...
    import perceptual_degradation_digital_canary_v5 as module
    if args.self_test:
        self_test(module)
    else:
