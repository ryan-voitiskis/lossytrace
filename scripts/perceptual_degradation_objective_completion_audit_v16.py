#!/usr/bin/env python3
"""Refresh objective audit after sparse successor nomination and confirmation freeze."""
...
PLAN_PATH = ROOT / "benchmarks/perceptual-degradation-v1/objective-completion-audit-plan-20260819-016.json"
REPORT_PATH = ROOT / "research/toolchains/evidence/perceptual-degradation-objective-completion-audit-20260819-016.json"
PLAN_ID = "perceptual-degradation-objective-completion-audit-20260819-016"
REPORT_ID = PLAN_ID
EXPECTED_BINDINGS = {
...
def validate_plan(plan: dict[str, Any]) -> list[str]:
    ...
    if plan.get("state") != "score_blind_completion_audit_refresh_after_sparse_successor_nomination_and_confirmation_freeze":
...
def build_report(plan: dict[str, Any], plan_path: Path = PLAN_PATH) -> dict[str, Any]:
    ...
    predecessor = _load_module(_bound(plan, "predecessor_audit_implementation"), "objective_v15")
    ...
    metadata = _load_module(_bound(plan, "metadata_result_implementation"), "metadata_search_v2_result")
    ...
    confirmation = _load_module(_bound(plan, "confirmation_implementation"), "sparse_confirmation_v2")
    ...
    next_gate = "Pass exact-head CI for the frozen one-member checkpoint, then acquire only Freesound sound 703342 through the official authenticated original-download route and run two one-worker private descriptor replays without playback or derived PCM retention."
    ...
    requirements["truth_bearing_source_manifest_feasible_and_frozen"].update({
        "state": "tonal_contrast_confirmed_sparse_successor_metadata_nominated_confirmation_frozen_audio_unopened_manifest_unfrozen",
        "satisfied": False,
        "evidence": [
            "predecessor_audit.requirements.truth_bearing_source_manifest",
            "metadata_search_v2_result.record_dispositions",
            "metadata_search_v2_result.selected_candidate_metadata",
            "exact_member_confirmation_v2_plan",
        ],
        "reason": "A bounded text-only primary-record audit stopped after Freesound sound 703342 met the unchanged metadata prerequisites, and a one-member descriptor-confirmation checkpoint is frozen. The provider original has not been acquired or measured, metadata is not descriptor evidence or trait truth, and assignment, relationships and partition allocation remain unfrozen.",
        "next_gate": next_gate,
    })
    requirements["difficult_natural_and_production_negatives_preserved_in_evaluation"].update({
        "state": "tonal_exact_contrast_confirmed_sparse_successor_nominated_confirmation_pending_evaluation_unallocated",
        "satisfied": False,
        "evidence": [
        ...
        "reason": "The tonal non-sparse technical contrast remains confirmed and one sparse non-tonal candidate is metadata-qualified, but its descriptor has not run and neither contrast is assigned or allocated to scientific evaluation.",
        "next_gate": next_gate,
    })
    ...
    report.update({
        ...
        "report_id": REPORT_ID,
        "state": "objective_incomplete_sparse_successor_nominated_confirmation_checkpoint_frozen_audio_unopened",
    })
    report["summary"].update({
        ...
        "sparse_non_tonal_eligible_successor_count": 1,
        "sparse_non_tonal_exact_member_confirmation_v2_audio_accessed": False,
        "sparse_non_tonal_exact_member_confirmation_v2_plan_frozen": True,
        "sparse_non_tonal_metadata_search_v2_primary_record_count": 9,
        "sparse_non_tonal_successor_metadata_candidate_count": 9,
        "sparse_non_tonal_successor_selected": True,
        "unsatisfied_count": 10,
    })
    report["evidence_checks"].update({
        "independent_sparse_and_tonal_contrasts_established": False,
        "source_manifest_allocation_performed": False,
        "source_successor_selected": True,
        "source_trait_assignment_performed": False,
        "source_trait_manifest_frozen": False,
        "sparse_non_tonal_eligible_successor_count": 1,
        "sparse_non_tonal_exact_member_confirmation_v2_audio_accessed": False,
        "sparse_non_tonal_exact_member_confirmation_v2_complete": False,
        "sparse_non_tonal_exact_member_confirmation_v2_plan_frozen": True,
        "sparse_non_tonal_metadata_search_v2_audio_accessed": False,
        "sparse_non_tonal_metadata_search_v2_complete": True,
        "sparse_non_tonal_metadata_search_v2_discovery_query_count": 4,
        "sparse_non_tonal_metadata_search_v2_primary_record_count": 9,
