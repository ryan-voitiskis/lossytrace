#!/usr/bin/env python3
"""Bound synthetic-only full-rate feasibility audit of unchanged alignment v3.
...
PLAN = ROOT / "benchmarks/perceptual-degradation-v1/alignment-runtime-feasibility-plan-20260908.json"
PLAN_ID = "alignment-runtime-feasibility-20260908-001"
...
STAGES = ("alignment_other", "channel_other", "envelope", "coarse_correlation", "sample_correlation",
          "gain", "drift_other", "drift_correlation", "structural_other", "structural_correlation")
...
BINDINGS = {
    ...
    "runner": "scripts/perceptual_degradation_alignment_runtime.py",
    "tests": "scripts/tests/test_perceptual_degradation_alignment_runtime.py",
}
...
def load_plan():
    ...
    for module in (alignment, alignment.legacy, alignment.topology, binding):
...
class StageTimer:
    ...
    def installed(self):
        originals = {name: getattr(alignment, name) for name in
                     ("_channel", "_rms_envelope", "correlate", "gain_diagnostics", "window_audit")}
        ...
        def correlation_stage(args, kwargs):
            if "structural_other" in self.stack:
                return "structural_correlation"
            if "drift_other" in self.stack:
                return "drift_correlation"
...
def validate_case(record, case):
    if not isinstance(record, dict) or set(record) != {"case", "frames", "sample_rate_hz", "instrumented", "outcome", "alignment_seconds",
                       "alignment", "stage_timing", "inputs_immutable", "perceptual_support"}:
...
def execute(parent):
    ...
    run_root = Path(tempfile.mkdtemp(prefix="alignment-runtime-", dir=parent))
    write_new(run_root / "execution.json", {"head": head, "plan_sha256": binding.sha_file(PLAN),
    ...
    for index in range(2):
        ...
        for case in CASES:
            try:
                ...
                stopped = True
            row = {"case": case, "outcome": "not_run_resource_stop", "alignment": None} if stopped else run_worker(case)
            rows.append(row)
    ...
    report.update({"execution_head": head, "plan_sha256": binding.sha_file(PLAN), "native_binding_edge_checks_passed": True})
