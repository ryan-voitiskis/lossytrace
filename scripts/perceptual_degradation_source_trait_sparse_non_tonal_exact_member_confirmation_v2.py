#!/usr/bin/env python3
"""Confirm the frozen sparse non-tonal successor without retaining decoded PCM."""
...
PLAN_PATH = ROOT / "benchmarks/perceptual-degradation-v1/source-trait-sparse-non-tonal-exact-member-confirmation-v2-plan.json"
PUBLIC_REPORT_PATH = ROOT / "research/toolchains/evidence/perceptual-degradation-source-trait-sparse-non-tonal-exact-member-confirmation-v2-20260819-001.json"
PLAN_ID = "perceptual-degradation-source-trait-sparse-non-tonal-exact-member-confirmation-v2-20260819-001"
REPORT_ID = PLAN_ID
...
def validate_plan(plan: dict[str, Any]) -> list[str]:
    ...
    if plan.get("state") != "one_exact_sparse_non_tonal_successor_confirmation_frozen_before_audio_access":
    ...
    if member.get("exact_member_id") != "freesound_sound_703342":
    ...
    if member.get("provider_record_url") != "https://freesound.org/people/AnthonyChan0/sounds/703342/":
    ...
    if member.get("expected_container") != {
        "bits_per_sample": 32,
        "channel_count": 2,
        "duration_seconds_maximum": "11.850000",
        "duration_seconds_minimum": "11.750000",
        "sample_format": "ieee_float_pcm",
        "sample_rate_hz": 192000,
    }:
        errors.append("expected container differs")
    if member.get("maximum_encoded_bytes") != 26214400:
    ...
    if not errors:
        ...
        failed = load_json(_binding_path(plan, "failed_exact_member_report"))
        failed_row = next(row for row in failed["source_observations"] if row["exact_member_id"] == "freesound_sound_856645")
        if failed_row["descriptor_confirmation_passed"] is not False:
            errors.append("predecessor abstention differs")
...
def parse_ieee_float_pcm_wav(value: bytes) -> tuple[dict[str, Any], list[list[float]], bytes]:
    ...
    if format_tag == 0xFFFE:
        if len(fmt) < 40 or fmt[24:40] != FLOAT_SUBFORMAT_GUID:
            raise ValueError("unsupported WAVE_FORMAT_EXTENSIBLE subformat")
        valid_bits = struct.unpack_from("<H", fmt, 18)[0]
        if valid_bits != 32:
            raise ValueError("valid bits differ from 32-bit float")
    elif format_tag != 3:
        raise ValueError("source is not IEEE float PCM")
    if channel_count not in (1, 2) or bits != 32:
        raise ValueError("unsupported float PCM channel count or bit depth")
    width = 4
    if block_align != channel_count * width or byte_rate != sample_rate * block_align:
        raise ValueError("inconsistent float PCM format fields")
    pcm = chunks[b"data"]
    ...
    for sample_index, (sample,) in enumerate(struct.iter_unpack("<f", pcm)):
        if not math.isfinite(sample):
            raise ValueError("float PCM contains a non-finite sample")
        channels[sample_index % channel_count].append(float(sample))
...
def _measure(descriptor: ModuleType, record: dict[str, Any], encoded: bytes) -> tuple[dict[str, Any], bytes]:
    ...
    facts, channels, pcm = parse_ieee_float_pcm_wav(encoded)
...
def _public_report(plan: dict[str, Any], private: dict[str, Any], plan_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "report_id": REPORT_ID,
        "schema_version": 1,
        "state": "one_exact_sparse_non_tonal_successor_score_free_descriptor_replays_complete",
    }
...
def run(
    ...
    descriptor = _load_module(_binding_path(plan, "descriptor_implementation"), "sparse_tonal_descriptor_v2")
    payloads: list[bytes] = []
    for _ in range(plan["resources"]["replay_count"]):
        with tempfile.TemporaryDirectory(prefix="lossytrace-sparse-nontonal-confirm-v2-"):
