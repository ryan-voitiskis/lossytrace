#!/usr/bin/env python3
"""Independently audit the sparse non-tonal exact-member v2 projection."""
...
REPORT_PATH = ROOT / "research/toolchains/evidence/perceptual-degradation-source-trait-sparse-non-tonal-exact-member-confirmation-v2-20260819-001.json"
AUDIT_PATH = ROOT / "research/toolchains/evidence/perceptual-degradation-source-trait-sparse-non-tonal-exact-member-confirmation-v2-audit-20260819-001.json"
RUNNER_PATH = ROOT / "scripts/perceptual_degradation_source_trait_sparse_non_tonal_exact_member_confirmation_v2.py"
REPORT_ID = "perceptual-degradation-source-trait-sparse-non-tonal-exact-member-confirmation-v2-20260819-001"
AUDIT_ID = "perceptual-degradation-source-trait-sparse-non-tonal-exact-member-confirmation-v2-audit-20260819-001"
...
def _validate_channel(channel: dict[str, Any], errors: list[str], label: str) -> None:
    ...
    if spectral_supported:
        ...
        allowed = {"supported": False}
        if set(spectral) not in ({"supported"}, {"supported", "active_frame_count"}):
            errors.append(f"{label} unsupported spectral inventory differs")
        if spectral.get("active_frame_count", 0) >= 3:
            errors.append(f"{label} unsupported spectral count differs")
        if not set(allowed).issubset(spectral):
...
def validate_report(report: dict[str, Any], plan: dict[str, Any] | None = None) -> list[str]:
    ...
    if report.get("state") != "one_exact_sparse_non_tonal_successor_score_free_descriptor_replays_complete":
    ...
    if report.get("nominated_exact_member_id") != plan.get("member", {}).get("exact_member_id"):
...
def build_audit(report: dict[str, Any], private_output_dir: Path | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "state": f"independent_projection_audit_complete_sparse_successor_{terminal}",
    }
