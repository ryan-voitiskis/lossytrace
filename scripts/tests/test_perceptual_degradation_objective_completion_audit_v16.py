...
SCRIPT = SCRIPTS / "perceptual_degradation_objective_completion_audit_v16.py"
SPEC = importlib.util.spec_from_file_location("objective_completion_audit_v16", SCRIPT)
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)


class ObjectiveCompletionAuditV16Test(unittest.TestCase):
    @classmethod
    ...
    def test_nomination_and_confirmation_readiness_are_bounded(self) -> None:
        summary = self.report["summary"]
        self.assertEqual(1, summary["sparse_non_tonal_eligible_successor_count"])
        self.assertTrue(summary["sparse_non_tonal_exact_member_confirmation_v2_plan_frozen"])
        self.assertFalse(summary["sparse_non_tonal_exact_member_confirmation_v2_audio_accessed"])
