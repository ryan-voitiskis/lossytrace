...
SCRIPT = ROOT / "scripts/perceptual_degradation_source_trait_sparse_non_tonal_exact_member_confirmation_v3.py"
SPEC = importlib.util.spec_from_file_location("sparse_non_tonal_exact_confirmation_v3", SCRIPT)
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)


def pcm24_wav_bytes(channels: list[list[int]], sample_rate: int, extensible: bool = False) -> bytes:
    ...
    if extensible:
        ...
        fmt = struct.pack("<HHIIHH", 1, channel_count, sample_rate, sample_rate * block_align, block_align, 24)
...
class SparseNonTonalExactMemberConfirmationV3Test(unittest.TestCase):
    @classmethod
    ...
    def test_integer_parser_supports_direct_and_extensible_24_bit(self) -> None:
        channels = [[-(1 << 23), -1, 0, 1, (1 << 23) - 1], [0, 1, -1, 2, -2]]
        for extensible in (False, True):
            facts, decoded, pcm = MODULE.parse_signed_24bit_pcm_wav(pcm24_wav_bytes(channels, 48000, extensible))
            self.assertEqual("signed_integer_pcm", facts["sample_format"])
            self.assertEqual(2, facts["channel_count"])
            self.assertEqual(5, facts["frame_count"])
            self.assertEqual(channels, decoded)
            self.assertEqual(30, len(pcm))

    def test_float_and_16_bit_pcm_fail_closed(self) -> None:
        float_encoded = bytearray(pcm24_wav_bytes([[0], [0]], 48000))
        struct.pack_into("<H", float_encoded, 20, 3)
        with self.assertRaisesRegex(ValueError, "not signed integer"):
            MODULE.parse_signed_24bit_pcm_wav(bytes(float_encoded))
        encoded = bytearray(pcm24_wav_bytes([[0], [0]], 48000))
        struct.pack_into("<H", encoded, 34, 16)
        with self.assertRaisesRegex(ValueError, "bit depth"):
            MODULE.parse_signed_24bit_pcm_wav(bytes(encoded))

    def test_frozen_descriptor_passes_focused_sparse_noise_fixture(self) -> None:
        sample_rate = 8000
        samples = sparse_noise(sample_rate * 4)
        encoded = pcm24_wav_bytes([samples, samples], sample_rate)
        ...
        descriptor = MODULE._load_module(MODULE._binding_path(self.plan, "descriptor_implementation"), "descriptor_test_v3")
    ...
    def test_two_replay_and_public_redaction_plumbing(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                self.assertTrue(json.loads(public_text)["decision"]["descriptor_confirmation_passed"])
        finally:
    ...
    def test_tampered_member_and_open_verdict_fail_closed(self) -> None:
        ...
        tampered["member"]["exact_member_id"] = "freesound_sound_703342"
