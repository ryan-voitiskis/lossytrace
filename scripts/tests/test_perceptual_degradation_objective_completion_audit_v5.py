...
SCRIPT = ROOT / "scripts/perceptual_degradation_objective_completion_audit_v5.py"
SPEC = importlib.util.spec_from_file_location("objective_completion_audit_v5", SCRIPT)
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)


class ObjectiveCompletionAuditV5Test(unittest.TestCase):
    @classmethod
    ...
    def test_synthetic_estimator_integration_has_no_scientific_promotion(self) -> None:
        summary = self.report["summary"]
        self.assertTrue(summary["oracle_drift_synthetic_estimator_integration_passed"])
        self.assertTrue(summary["oracle_drift_estimator_integrated_on_synthetic_pcm"])
        self.assertFalse(summary["oracle_drift_estimator_integrated_on_retained_audio"])
        self.assertEqual(7, summary["oracle_drift_synthetic_case_count"])
        self.assertEqual(4, summary["oracle_drift_synthetic_applied_case_count"])
        self.assertEqual(2, summary["oracle_drift_synthetic_nonzero_no_apply_case_count"])
        self.assertEqual(13, summary["oracle_drift_synthetic_gate_pass_count"])
        self.assertFalse(summary["oracle_drift_retained_validation_complete"])

        checks = self.report["evidence_checks"]
        self.assertTrue(checks["oracle_drift_training_selection_held_out_apply_integrated"])
        self.assertTrue(checks["oracle_drift_exact_resampler_integrated_after_estimation"])
        ...
        self.assertFalse(
            checks["oracle_drift_synthetic_estimator_is_full_reference_validation"]
        )
    ...
    def test_full_reference_oracle_requirement_remains_unsatisfied(self) -> None:
        ...
        self.assertIn("All thirteen gates passed", requirement["reason"])
    ...
    def test_authority_and_promotion_policy_mutations_fail_closed(self) -> None:
        ...
        plan["completion_policy"][
            "synthetic_drift_estimator_integration_counts_as_full_reference_oracle_pass"
        ] = True
        plan["completion_policy"][
