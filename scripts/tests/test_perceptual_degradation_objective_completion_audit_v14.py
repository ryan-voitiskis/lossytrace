...
SCRIPT = ROOT / "scripts/perceptual_degradation_objective_completion_audit_v14.py"
SPEC = importlib.util.spec_from_file_location("objective_v14", SCRIPT)
assert SPEC and SPEC.loader
MODULE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(MODULE)


class ObjectiveCompletionAuditV14Test(unittest.TestCase):
    @classmethod
    ...
    def test_rigorous_negative_is_preserved_without_promotion(self) -> None:
        summary = self.report["summary"]
        self.assertTrue(summary["sparse_tonal_exact_member_confirmation_complete"])
        self.assertEqual(2, summary["sparse_tonal_exact_member_count"])
        self.assertEqual(1, summary["sparse_tonal_exact_member_confirmation_pass_count"])
        self.assertTrue(summary["sparse_non_tonal_candidate_abstained"])
        self.assertTrue(summary["tonal_non_sparse_candidate_confirmed"])
    ...
    def test_no_trait_or_manifest_allocation_is_promoted(self) -> None:
        checks = self.report["evidence_checks"]
        self.assertFalse(checks["source_trait_assignment_performed"])
        self.assertFalse(checks["source_manifest_allocation_performed"])
        self.assertFalse(checks["source_trait_manifest_frozen"])
        self.assertTrue(checks["sparse_tonal_private_replays_byte_identical"])
        self.assertTrue(checks["sparse_tonal_public_projection_independently_audited"])
