#!/usr/bin/env python3
"""Run the frozen v29 independent external transfer with safe resume."""
...
BASE_PATH = (
    ROOT / "scripts" / "run-audio-integrity-frozen-external-transfer.py"
)
SPEC = importlib.util.spec_from_file_location(
    "audio_integrity_external_runner_base",
    BASE_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise SystemExit(f"cannot load external runner base: {BASE_PATH}")
...
def validate_contract(
    *,
    ...
    if (
        ...
        or tools.get("base_external_runner_sha256")
        != BASE.sha256_file(BASE_PATH)
        or tools.get("benchmark_runner_sha256")
        != BASE.sha256_file(runner)
        ...
        raise SystemExit("frozen independent runner contract differs")
    if (
        BASE.load_json(manifest_path.parent / "corpus-plan.json") != plan
        or BASE.load_json(
            manifest_path.parent / "candidate-precommit.json"
        )
        != precommit
